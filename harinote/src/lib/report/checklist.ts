 ...
 *
 * 여기는 "가방에 넣을 것"만 담는다 — 현장에서 조심할 일(계곡 수위, 화기 사용,
 * 아이 컨디션)은 값과 날짜를 담아 설명해야 뜻이 통하므로 cautions.ts가 맡는다.
 *
...
export function buildPlanChecklist(
  ...
  if (has("wind")) {
    items.push(
      has("wind", ["outdoor_mountain", "outdoor_coast"])
        ? "바람막이 준비하기"
        : "바람에 날리기 쉬운 물품 고정, 겉옷 준비하기",
    );
  }
  if (has("pm")) {
    items.push("보건용 마스크(KF80 이상) 챙기기");
